...
class ProductManagement implements ProductManagementInterface
{
     ...
     * @return array
     */
    private function getActiveAttributes(array $productAttOptions): array
    {
        $activeAttributes = [];

        foreach ($productAttOptions as $attributeData) {
            if (!empty($attributeData['values'][0]['value_index'])) {
                $activeAttributes[$attributeData['attribute_code']] = $attributeData['values'][0]['value_index'];
            }
        }
    ...
    private function getActiveSelections(array $selectionArr): array
    {
        ...
        foreach ($selectionArr as $optionId => $selections) {
            $optionData = [
                'id' => $optionId,
                'selection_value' => []
            ];
            ...
            foreach ($selections as $selection) {
                $optionData['selection_value'][] = [
                    'qty' => $selection['selection_product_qty'],
                    'value_id' => $selection['selection_id'],
                    'change_qty' => $selection['selection_can_change_qty'],
                    'value' => (bool)$selection['selection_is_default']
                ];
            }
    ...
    {
        foreach ($activeAttributes as $attributeCode => $attributeValue) {
            foreach ($usedProducts as $usedProduct) {
                if (!empty($usedProduct[$attributeCode]) && $usedProduct[$attributeCode] === $attributeValue) {
                    return $usedProduct;
                }
            }
        }
    ...
    /**
     * Get price configuration for an option
     *
     * @param Value|Option $option
     * @return array
     */
    protected function getPriceConfiguration(Value|Option $option): array
    {
        ...
        return [
            'prices' => [
                'oldPrice' => ['amount' => $this->convertCurrency->execute((string)$option->getRegularPrice(), $this->currencyCode)],
                'basePrice' => ['amount' => $this->catalogData->getTaxPrice($product, $optionPrice, false)],
                'finalPrice' => ['amount' => $this->catalogData->getTaxPrice($product, $optionPrice, true)],
            ],
